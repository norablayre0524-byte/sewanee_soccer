...
# Renaming columns so they are more clear
data<-data%>%
  rename(Type=fwrd_pass,
         Status=complete,
         third='attacking third')

# Filtering the attacking data to only passes
pass_third_data<-data%>%
  filter(Type %in% c("fwrd_pass","bck_pass"))%>%
  group_by(third, Type)%>%
  summarise(count=n(), .groups="drop")

# Reordering the data entries so that they are more intuitive on plots
...
```{r, echo=FALSE}
# Putting the numbers for wins, losses, and ties in recent years for Sewanee Men's
# soccer into a data frame
record<- data.frame(
...
# turning that data frame into a plot
...
# Plotting the number of fowards and back passes by Sewanee to understand where they take
# their passing risks on the field
ggplot(pass_third_data, aes(
  x=third,
  y=count,
  fill=Type))+
  geom_col(stat="identity",position="dodge")+ # lets the bars be right next to each other rather than stacked
  labs(
    title="The Passing Blueprint", # Adding title, subtitle, caption, and axis lables
    subtitle="Number of Passes in Each Third",
    x="Third of the Field",
    y="Number of Passes",
    fill="Type of Pass",
    caption="Note: fwrd means foward and bck means backwards"
  )+
  scale_fill_manual(
    values=c("fwrd_pass"='blue',"bck_pass"='black') #color coding the types of passes for visual clarity
  )+
  theme_pander() # adding a clean theme and attempting to eliminate clunky features that
# limit the clarity of the plot
...
# Grouping the pass and dribbling stats by third of the field
pass_dribble_rate<-pass_dribble_data%>%
  group_by(third, action)%>%
  summarise(completion_rate = mean(completion), .groups="drop")

# Changing the order of data enties for easier graph interpretation
...
# Plotting the pass success rate and dribble success rate in each thrid of the field
ggplot(pass_dribble_rate, aes(
  x=third,
  y=completion_rate,
  fill=action
  ))+
  geom_col(stat="identity", position=position_dodge(preserve="single"))+ # Putting bars side-by-side instead of stacked; preserve ="single" leaves space on chart where there is no data 
  labs(
    title="Angle of Attack", # Adding plot labels
    subtitle="Pass and Dribble Competion by Third",
    caption="There were no dribbles attempted in the defensive third", # Acknowledging why a part of the graph has no data
    x="Third of the Field",
    y="Completion Rate",
    color="Action"
  )+
  scale_fill_manual(
    values= c("pass"='purple',"dribble"= 'gold') # Adding school themed colors
  )+
  theme_pander() # Putting a consistent theme
```
...
# Filtering the data to foward passes only
ground_air_data<-pass_dribble_data%>%
  filter(Type == 'fwrd_pass')%>%
  group_by(third, ground)%>% # Grouping data together
  summarise(count = n(), .groups="drop")%>% # Allowing data entries to be shown numerically
  mutate(third = factor(third, levels = c('attacking third','middle third', 'defensive third')), # logically reordering data columns
         ground = factor(ground, levels = c('ground','air'))) 

# Making a plot showing Sewanee's passing on the ground and the air
ggplot(ground_air_data, aes(
  x=third,
  y=count,
  fill=ground
))+
  geom_col(stat="identity", position=position_dodge(preserve="single"))+ # Ensuring proper bar formatting
  labs(
    title="Altitude of Attack",  # Adding plot labels
  ...
  scale_fill_manual(values=
                      c("ground"='turquoise',"air"='magenta'))+ # Adding color
  theme_pander() # Keeping the theme consistent
...
# Grouping data together and allowing it to be shown numerically
...
# Reordering the third of the field column
pass_defense_filter$third<- factor(pass_defense_filter$third, levels = c('attacking third','middle third', 'defensive third'))

# Making the plot showing the number of complete and incomplete passes allowed by Sewanee
ggplot(pass_defense_filter, aes(
  x=third,
  y=count,
  fill=success
))+
  geom_col(stat="identity", position="dodge")+ # Ensuring proper bar formatting
  scale_fill_manual(
    values = c(complete='red',incomplete='green') # Giving the graph intuitive color
  )+
  labs(
    title="Tactics Against Passing", # Adding plot labels
    subtitle="Number of Passes for the Opposition",
    x="Third of the Field",
    y="Number of Passes",
    fill="Key",
    caption="High completed pass numbers by opposition indicates defensive flaws"
  )+
  theme_pander() # Keeping the theme consistent
```
...
# Filtering the data to tackles
tackle_data<-defense_data%>%
  filter(action == 'tackle')

# Allowing the data to be shown numerically and reordering columns
...
# Making the plot for tackles won by Sewanee
ggplot(tackle_count_data, aes(
  x=third,
  y=count,
  fill=success
))+
  geom_col(stat="identity", position=position_dodge(preserve="single"))+ # Bar formatting again
  scale_fill_manual(
    values = c(won='green','not won'='red') # Adding intuitive color
  )+
  labs(
    title="Defensive Success on the Ground", # Adding plot laels
...
# Filtering the data for headers
...
# Allowing the data to be interpreted numerically
header_count_data<-header_data_filter%>%
  group_by(success, third)%>%
  summarise(count = n(), .groups="drop")

# Making the plot for headers won by Sewanee
ggplot(header_count_data, aes(
  x=third,
  y=count,
  fill=success
))+
  geom_col(stat="identity",position="dodge")+ # Bar formatting again
  scale_fill_manual(
    values = c(won='green','not won'='red') # Adding intuitive color
  )+
  scale_y_continuous(limits = c(0,4))+ # Changing the scale of the y-axis so the data significance is more clear
  labs(
    title="Defensive Success in the Air", # Adding plot labels
...
## Looking Towards the Future {width=500}
